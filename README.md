...
**Bootstrap probes** (`bootstrap/`, see `BOOTSTRAP.md`) — verifying the floor of the
stack: `stage0-posix` (526-byte seed ⇒ M2-Planet C compiler, no host compiler),
`gcc-from-source` (GCC 14.2), `musl-from-source` (libc), `tcc-selfhost` (tcc built by
tcc), `tcc-builds-lua` (tcc ⇒ lua). All build + smoke-test in a container.
